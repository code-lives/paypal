...
class PaymentModel extends Model
{
    ...
    public function payment($config, $data)
    {
        ...
        switch ($payment_way) {
                //支付宝H5
            case 'alipay_h5':
                $aop = new \wxalipay\paypal\AlipayAop\AopClient($config);
                $request = new \wxalipay\paypal\AlipayAop\request\AlipayTradeWapPayRequest();
            ...
            case 'alipay_app':
                $aop = new \wxalipay\paypal\AlipayAop\AopClient($config);
                $request = new \wxalipay\paypal\AlipayAop\request\AlipayTradeAppPayRequest();
                $setBizContent = json_encode(
                    [
                        'body'            => $payment_name,
                        'subject'         => $payment_name,
            ...
            case 'alipay_pc':
                $aop = new \wxalipay\paypal\AlipayAop\AopClient($config);
                $request = new \wxalipay\paypal\AlipayAop\request\AlipayTradePagePayRequest();
                $setBizContent = json_encode(
                    [
                        'body'            => $payment_name,
                        'subject'         => $payment_name,
            ...
            case 'wxpay_app':
                ...
                $weiXinPay = new \wxalipay\paypal\Wxpay\WxPay($config);
    ...
    public function searchPaymentResult($config, $payment_way, $out_sn = '', $trade_no = '')
    {
        ...
        switch ($payment_way) {
            ...
            case 'alipay_pc': //支付宝PC
                $aop = new \wxalipay\paypal\AlipayAop\AopClient($config);
                $request = new \wxalipay\paypal\AlipayAop\request\AlipayTradeQueryRequest();
                if ($out_sn) {
    ...
    public function aliPaySearchRefund($config, $param)
    {
        ...
        $aop = new \wxalipay\paypal\AlipayAop\AopClient($config);
        $request = new \wxalipay\paypal\AlipayAop\request\AlipayTradeFastpayRefundQueryRequest();
        $setBizContent = json_encode(
            [
    ...
    public function aliPayRefund($config, $param)
    {
        ...
        $aop = new \wxalipay\paypal\AlipayAop\AopClient($config);
        $request = new \wxalipay\paypal\AlipayAop\request\AlipayTradeRefundRequest();
        $setBizContent = json_encode(
            [
                'out_trade_no'   => $param['out_sn'],
                'trade_no'       => $param['trade_no'],
    ...
    public function getBillDownload($config, $param)
    {
        ...
        if (in_array($param['payment_way'], ['alipay_app', 'alipay_h5', 'alipay_pc'])) {
            ...
            $aop = new \wxalipay\paypal\AlipayAop\AopClient($config);
            $request = new \wxalipay\paypal\AlipayAop\request\AlipayDataDataserviceBillDownloadurlQueryRequest();
